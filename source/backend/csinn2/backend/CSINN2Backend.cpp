...
namespace MNN {
    ...
    void CSINN2Backend::setNetworkInput(const std::vector<Tensor *> &inputs, const Op* op) {
    ...
    ErrorCode CSINN2Backend::bulidIRModelAndLoad() {
    ...
    shared_ptr<ge::Operator> CSINN2Backend::getInputOps(const Op *op, int index) {
    ...
    void CSINN2Backend::setOutputOps(const Op *op, vector<shared_ptr<ge::Operator>>&& HIAI_op,
    ...
    static const auto __npu_global_initializer = []() {
        MNNInsertExtraRuntimeCreator(MNN_FORWARD_USER_0, new CSINN2BackendCreator, true);
        return true;
    }();
}
